...
export const LoadAssetsTask = (assets: number[]): Promise<TaskResult> => {
  const tasks: Promise<void>[] = assets.map(
    (source: number): Promise<void> => {
...
export const AppLoading: React.FC<ApplicationLoaderProps> = props => {
  ...
  const startTasks = (): Promise<void> => {
    return Promise.all(props.tasks.map(task => task().then(saveTaskResult))).then();
  };

  const renderLoadingElement = (): React.ReactElement => (
    <ExpoAppLoading autoHideSplash={false} startAsync={startTasks} onFinish={onTasksFinish} onError={console.error} />
  );
